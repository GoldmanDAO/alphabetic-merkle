...
use sea_orm::{
    error::DbErr, ActiveModelTrait, DatabaseConnection, EntityTrait, PaginatorTrait, QueryOrder,
...
use merkletree::{
    ...
    merkle_tree::{get_merkle_root, Error},
};

async fn list_proposals_paginated(
    db: &DatabaseConnection,
    pag: Pagination,
) -> Result<Vec<ProposalsModel>, DbErr> {
    if !pag.check_range() {
...
pub async fn get_proposals_by_id(
    ...
    match proposal_res {
        Ok(proposal_opt) => match proposal_opt {
            ...
            None => Err(DbErr::Query(RuntimeErr::SqlxError(
...
pub async fn get_proposal_with_accounts(
    db: &DatabaseConnection,
    id: i32,
) -> Result<ProposalsModel, DbErr> {
    ...
    proposal_res
        .first()
        .ok_or(DbErr::Query(RuntimeErr::SqlxError(
            sqlx::error::Error::RowNotFound,
        )))
...
fn get_merkletree_root(accounts: Vec<AccountsActiveModel>) -> Result<String, Error> {
    let accounts: Vec<AccountWithBalance> = accounts
        .iter()
        .map(|account| {
            ...
            match (address, balance) {
                (Some(address), Some(balance)) => {
                    AccountWithBalance::new(address.as_str(), balance.as_str())
                }
...
pub async fn insert_proposal(
    db: &DatabaseConnection,
    mut proposal_data: ProposalsActiveModel,
    account_models: Vec<AccountsActiveModel>,
) -> Result<i32, DbErr> {
    let proposal_id = db
        .transaction::<_, Option<i32>, DbErr>(|txn| {
            Box::pin(async move {
                ...
                let proposal: ProposalsActiveModel = proposal_data.save(txn).await?;

                accounts.iter_mut().for_each(|account_model| {
                    account_model.set(
                        AccountsBase::Column::ProposalId,
                        proposal.id.clone().into_value().unwrap(),
                    );
                });
